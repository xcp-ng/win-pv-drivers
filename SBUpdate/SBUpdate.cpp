...
static bool FindFveOsDrive(_In_ IWbemServices* services, _Outptr_ IWbemClassObject** outObject) {
    *outObject = nullptr;
    ...
    HRESULT hr = CoSetProxyBlanket(services, RPC_C_AUTHN_WINNT, RPC_C_AUTHZ_NONE, NULL, RPC_C_AUTHN_LEVEL_CALL, RPC_C_IMP_LEVEL_IMPERSONATE, NULL, EOAC_NONE);
    if (FAILED(hr)) {
        throw std::system_error(hr, std::system_category(), "CoSetProxyBlanket");
    }

    CComPtr<IEnumWbemClassObject> enumerator;
    hr = services->ExecQuery(_bstr_t(L"WQL"), _bstr_t(L"select * from Win32_EncryptableVolume where VolumeType = 0"), WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &enumerator);
    if (FAILED(hr)) {
...
static bool FveIsSecureBootBound(_In_ IWbemServices* services, _In_ IWbemClassObject* win32_EncryptableVolumeClass, _In_ BSTR instancePath) {
    HRESULT hr;

    CComPtr<IWbemClassObject> getSecureBootBindingState_Out;
    hr = win32_EncryptableVolumeClass->GetMethod(FveGetSecureBootBindingState, 0, nullptr, &getSecureBootBindingState_Out);
    if (FAILED(hr)) {
    ...
    hr = services->ExecMethod(instancePath, _bstr_t(FveGetSecureBootBindingState), 0, nullptr, nullptr, &outParams, &result);
    if (FAILED(hr)) {
...
static void FveSuspend(_In_ IWbemServices* services, _In_ IWbemClassObject* win32_EncryptableVolumeClass, BSTR fveInstancePath) {
    HRESULT hr;
    ...
    if (FAILED(hr)) {
        throw std::system_error(hr, std::system_category(), "win32_EncryptableVolumeClass->GetMethod(DisableKeyProtectors)");
    }

    CComPtr<IWbemCallResult> result;
    hr = services->ExecMethod(fveInstancePath, _bstr_t(FveDisableKeyProtectors), 0, nullptr, disableKeyProtectors_In, nullptr, &result);
    if (FAILED(hr)) {
